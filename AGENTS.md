...
- **Device liveness & status**: `hooks/use-realtime-device-status.ts` subscribes to `/devices` via `onValue`. Firmware sends `scanMode` inside heartbeat payload. Web changes mode by writing to `deviceCommands/{deviceId}/scanMode`, which the device polls at a 2.5s interval in its main loop (gated/bypassed during OTA validation to prevent rollback boot loops). Local state synchronization is handled via CustomEvents to sync `scanMode` across instances on the same page. If a device goes offline (lastSeen age > 9s, evaluated every 1s), the dashboard resets scanMode locally and on Firebase commands to "Manual" ("ask").
- **Auto stock mode**: When scanner mode is set to `in` or `out` on the dashboard, scanned barcodes matching existing inventory automatically adjust stock by +1 or -1 directly via `adjustStock` and display a silent toast notification, skipping the Quick Action popup modal.
