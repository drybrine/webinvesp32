...
- **ESP32 → Firebase**: firmware PUTs directly to `/devices/{deviceId}` (heartbeat every ~5s, includes batteryLevel/rssi) and pushes scans to `/scans/{id}`. Firmware also reads `/inventory` to display product info on its OLED.
...
- There is no standalone `/scan` or `/audit` page. ESP32 scan events still flow through the realtime provider, and admin audit records still persist to `/auditLogs`.
...
Primary prediction runs on the Python serverless function `api/predict.py`, with `lib/stock-prediction.ts` as the TypeScript fallback for local/failed API calls. The model mirrors the notebook `model_prediksi_stok_linear_regression.ipynb`: **simple linear regression (lag-1) on EMA-smoothed daily consumption**. Reactive stock series `buildDailySeriesFromTransactions` reconstructs daily stock from ALL transactions (`in`/`out`/`adjustment`) given the current quantity; consumption is derived per transaction-gap (`stockDelta / gapDays`), smoothed with EMA (alpha 0.05), then fit as `Y = a + b·X` (X = yesterday's smoothed consumption, Y = today's). Forecast is iterative: `S_{d+1} = max(0, S_d − nextConsumption)`. Metrics (MAE/RMSE/MAPE/R²) are computed **in-sample on the full series** (model fit from the training split), so `R²` stays in `0..1` and never goes negative. This matches PowerPoint/notebook values. Minimum 2 stock points to fit — callers must guard. The `/prediksi` page displays only regression/forecast outputs needed for the thesis; if the API returns anomaly metadata, frontend code should ignore it unless the thesis scope changes.
...
`next.config.mjs` has custom `splitChunks` with `maxSize: 50000` and `vendorSplit` that splits node_modules per-package. This previously shattered `recharts` across 7 chunks and broke cross-chunk references (`s(...) is not a function`). The fix was to render prediction charts as hand-built SVG (`components/prediction-chart.tsx`) instead of recharts. `recharts` and `components/ui/chart.tsx` (the shadcn recharts wrapper) were removed entirely. If you add a library with heavy internal cross-module references, test a production build, not just dev.
